(function() {
	...
	var module = angular.module('Holzzuschnitt.controllers', []);

	module.controller('CanvasCtrl', ['$scope',
		function CanvasCtrl($scope) {
			...
			$scope.addData = function() {
				...
				if ($scope.data.length > 0) {
					id = $scope.data[$scope.data.length - 1].id +
						1;
				}
				var p = {
					id: id,
			...
			function drawDot(data) {
				context.beginPath();
				context.arc(data.x, data.y, data.amount, 0, 2 *
					Math.PI, false);
